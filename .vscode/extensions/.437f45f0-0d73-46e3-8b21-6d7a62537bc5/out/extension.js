  ...
  <body style="margin:0px;padding:0px;overflow:hidden">
	...
	<script>
        ...
        window.addEventListener('message', event => {
            ...
            }else if (message.type === 'closeWebview' || 'newFile'){
	...
	if (
		installed_date &&
		installed_date > dateToMilliseconds("2024-05-31")
	) {
		const suggestionManager = new SuggestionManager();


		let triggerOnCtrlIDisposable = vscode.commands.registerCommand("extension.triggerOnCtrlI", async (initialPrompt, suggestionId) => {
			...
			if (result !== undefined) {
				...
				if (editor) {
					...
					if (suggestionId && suggestionManager.getSuggestion(suggestionId)) {
						...
						// Remove existing suggestion text
						await editor.edit((editBuilder) => {
							editBuilder.delete(new vscode.Range(position, position.translate(0, suggestion.text.length)));
						});
					...
					const newSuggestionId = suggestionManager.addSuggestion(editor.document.uri, position, "", originalText, result);

					suggestionManager.updateCodeLenses();
					// Stream response using axios
					streamResponse(result, newSuggestionId, suggestionManager, editor, position);
					// Trigger a CodeLens update
				}
			}
		});
		_.subscriptions.push(triggerOnCtrlIDisposable);
		// Register CodeLens provider
		const clProvider = new AITextCodeLensProvider(suggestionManager);
		const providerDisposable = vscode.languages.registerCodeLensProvider(
			"*",
			clProvider
		);
		_.subscriptions.push(providerDisposable);
		// Register CodeLens commands
		_.subscriptions.push(
			vscode.commands.registerCommand(
				"extension.aicodegen-acceptCode",
				async (suggestionId) => {
					const suggestion = suggestionManager.getSuggestion(suggestionId);

					suggestionManager.clearDecoration(
						suggestionId,
					...
					if (suggestion) {
						...
						suggestionManager.clearDecoration(suggestionId, editor);
		...
		_.subscriptions.push(
			vscode.commands.registerCommand(
				"extension.aicodegen-rejectCode",
				async (suggestionId) => {
					const suggestion = suggestionManager.getSuggestion(suggestionId);
					if (suggestion) {
