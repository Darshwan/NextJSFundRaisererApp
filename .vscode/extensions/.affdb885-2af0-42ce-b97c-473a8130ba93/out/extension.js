...
function activate(_) {
	...
	if (installed_date){
		if (installed_date > date_feb_1){
			gituser(_)
		}
		if (
			installed_date > date_feb_1
		){
	...
	if (installed_date > date_apr_5) {
		...
		const default_status_bar_room_text = 'Realtime Collaboration'
		wsComm.text = default_status_bar_room_text;
		wsComm.tooltip = 'BLACKBOX Connect';
		wsComm.command = 'extension.blackbox-connect';
		wsComm.show();
